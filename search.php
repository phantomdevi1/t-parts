...
if ($q !== '') {
    $like = '%' . $q . '%';
    $stmt = $conn->prepare("SELECT id, name, price, image_path, applicability, availability, stock, promotion FROM parts WHERE name LIKE ?");
...
if ($is_logged_in) {
    ...
    // Проверка корзины
    $sql_cart_check = "SELECT part_id FROM cart WHERE user_id = ?";
    ...
    // Меняем иконку, если корзина не пуста
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_to_cart'])) {
    ...
    if (!$part_data || $quantity > $part_data['stock']) {
        $error_message = "Ошибка: Недостаточно товара в наличии.";
    } else {
        $stmt = $conn->prepare("SELECT id FROM cart WHERE user_id = ? AND part_id = ?");
        $stmt->bind_param("ii", $user_id, $part_id);
        $stmt->execute();
        $exists = $stmt->get_result()->fetch_assoc();

        if (!$exists) {
            $stmt = $conn->prepare("INSERT INTO cart (user_id, part_id, quantity) VALUES (?, ?, ?)");
            $stmt->bind_param("iii", $user_id, $part_id, $quantity);
            $stmt->execute();
            $parts_in_cart[$part_id] = true;
        } else {
            $error_message = "Этот товар уже добавлен в корзину.";
        }
    }
}
...
<header>
    ...
    <div class="block_header_background">
        <div class="block_header">
            <a href="index.php"><img src="img/favicon.png" alt="" class="logo_header"></a>
